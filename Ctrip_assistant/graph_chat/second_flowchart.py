...
from Live_Streaming_practice.Ctrip_assistant.graph_chat.assistant import create_assistant_node, sensitive_tools, \
    safe_tools, sensitive_tools_name
...
def route_conditional_tools(state: State) -> str:
    """
    Decide which node to execute based on current state.
    :param state: current state
    :return: string of the next node
    """
    next_node = tools_condition(state) #tools_condition is the detault langgraph route, it tells whether next node is 'tools' or END
    if next_node == END:
        return END

    ai_message = state['messages'][-1]
    tool_call = ai_message.tool_calls[0]
    if tool_call['name'] in sensitive_tools_name:
...
builder.add_conditional_edges(
    ...
    #if the name returns is identical to the node name, no need to use dictionary like {'safe_tools' : 'safe_tools'}
)
